...
export default ({
	...
	const submitHandler = async () => {
		...
		const filteredAnswerBodies = answerBodies.filter((answerBody) => {
			if (
				answerBody.type === "paragraph" ||
				answerBody.type === "header"
			) {
						...
						{answerBodies.map((body, index) => {
							switch (body.type) {
								case "header":
									return (
										<div
											className="basis-full flex flex-row mb-5"
											key={index}
										>
											<div className="basis-5/6">
												<InvisibleTextArea
													...
													placeholder={"Header"}
													onTextChange={(text) =>
														headerOrParagraphChangeHandler(
															text,
															index,
														)
													}
				...
				>
					...
					>
						<Alert
							...
							sx={{
								width: "100%",
								border: `1px solid ${palette.wrong}`,
							}}
						>
							{errorMessage}
						</Alert>
